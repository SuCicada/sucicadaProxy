...
from flask import Flask, Blueprint, request, Response, url_for, redirect
...
@proxy.route('/proxy/<int:port>/', methods=["GET", "POST", "PUT", "DELETE"])
@proxy.route('/proxy/<int:port>/<path:file>', methods=["GET", "POST", "PUT", "DELETE"])
def my_proxy_request(port, file=""):
    proto = 'http'
    host = f'{TARGET_HOST}:{port}'
    # url = url_for("proxy_request", proto=proto, host=host, file=file)
    url = f'{request.host_url}proxy/{proto}/{host}/{file}'
    print(url)
    # res = requests.request(request.method, url, data=request.json, headers=request.headers)
    # return Response(res)
    # return redirect(url, code=301)
    return proxy_request(proto, host, file)


# For RESTful Service
@proxy.route('/proxy/v0/<proto>/<host>/', methods=["GET", "POST", "PUT", "DELETE"])
@proxy.route('/proxy/v0/<proto>/<host>/<path:file>', methods=["GET", "POST", "PUT", "DELETE"])
def proxy_request(proto, host, file=""):
    hostname, port = parse_host_port(host, proto)
    # proto = 'http'
    # hostname, port = TARGET_HOST, port
    ...
    if proto.lower() == 'http':
        request_method = httplib.HTTPConnection
    print(hostname, port)
    print(path)
